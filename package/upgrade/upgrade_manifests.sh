...
migrate_longhorn_v1beta1_crds() {
  ...
  echo "Checking version, it only happened on v1.6.1 -> v1.7.x upgrade"
  if [[ $UPGRADE_PREVIOUS_VERSION != "v1.6.1" ]]; then
  ...
  echo "Starting migration of CRDs that store v1beta1 resources..."
  migration_time="$(date +%Y-%m-%dT%H:%M:%S)"
  crds=($(kubectl get crd -l app.kubernetes.io/name=longhorn -o json | jq -r '.items[] | select(.status.storedVersions | index("v1beta2")) | .metadata.name'))
  for crd in "${crds[@]}"; do
    echo "Migrating ${crd} ..."
