...
type ImageActionHandler struct {
...
func (h ImageActionHandler) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
...
func (h ImageActionHandler) do(rw http.ResponseWriter, req *http.Request) error {
	vars := mux.Vars(req)
	action := vars["action"]
	switch action {
	case actionUpload:
...
func (h ImageActionHandler) waitForBackingImageDataSourceReady(name string) error {
	retry := 30
	for i := 0; i < retry; i++ {
...
func (h ImageActionHandler) updateImportedConditionOnConflict(image *apisv1beta1.VirtualMachineImage,
